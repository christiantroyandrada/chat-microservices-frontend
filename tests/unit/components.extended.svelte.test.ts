...
describe('Extended component tests', () => {
	...
	describe('ChatList', () => {
		it('opens create modal, searches, selects user and calls onCreate', async () => {
			...
			// open modal
			const newChatBtn = container.querySelector(
	...
	describe('MessageInput', () => {
		...
		it('blocks send when message over maxLength and shows toast', async () => {
			...
			expect(
				(toastStore.error as unknown as ReturnType<typeof vi.fn>).mock.calls.length
			).toBeGreaterThan(0);
		});
	});

	describe('MessageList', () => {
		it('renders messages and date separators', async () => {
